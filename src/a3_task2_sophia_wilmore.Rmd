...
*"X" is used to here to indicate that a task has been completed*

- Before moving on, click **File > Save As** to save a copy of this .Rmd (also to the `src` folder in this project) that is updated with your first and last name in the file name (`a3_task2_firstname_lastname.Rmd`) X 

- Once you have saved your copy of the .Rmd, you can delete the generic original (if you want...or just leave it there) X

- Run the setup chunk to get the `tidyverse` & `lubridate` packages attached (`lubridate` is a package to make it a bit easier to work with dates & times) X
...
# The name in row 49 was only separated into two names when there were three, the very last name was dropped
```
...
```{r}
# Your code here: 
some_dates <- c(02182000, 08202100, 06301970)
...
gasol_hookshots <- lakers %>% 
  filter(player == "Pau Gasol",
         type == "hook") %>% 
  mutate(date_new = ymd(date),
         month_new = month(date_new))

gasol_hookshots_by_month <- gasol_hookshots %>% 
  group_by(type, month_new) %>%
  count(month_new, name = "hookshots_by_month")

gasol_hookshots_by_month
